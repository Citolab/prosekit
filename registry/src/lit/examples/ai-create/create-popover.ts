...
import type { AiCreateRequestDetail, AiPrompt } from './prompts'
...
class LitAiCreateResult extends LitElement {
  ...
  private async start(detail: AiCreateRequestDetail) {
    ...
    if (!apiKey || !endpoint) {
      window.alert(
        'No API endpoint or key found. Open the Stream Content story first and configure both — they are stored in localStorage and reused here.',
      )
      return
    }
  ...
  override render() {
    ...
    return html`
      <div
        ${ref(this.popoverRef)}
        popover="manual"
        class="ai-create-popover flex flex-col gap-2 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 shadow-lg"
      ...
      >
        ${s
          ? html`
              ...
              <div
                class="ai-create-preview max-h-64 overflow-auto rounded border border-gray-200 dark:border-gray-800 p-2 text-sm"
              ...
              <div class="flex gap-2 justify-end">
                <button
                  type="button"
                  @click=${this.cancel}
                  class="px-3 py-1 rounded text-sm border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800"
                >
                  Cancel
                </button>
                <button
                  ...
                  class="px-3 py-1 rounded text-sm border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50"
                >
                  Try again
                </button>
                <button
