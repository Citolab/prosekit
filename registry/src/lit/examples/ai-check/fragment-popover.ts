    ...
    if (!this.removeUpdateExtension) {
      this.removeUpdateExtension = editor.use(
        defineUpdateHandler((view) => {
          // Close if the active fragment is no longer present.
          if (this.state) {
  ...
  override render() {
    return html`
      <div
        ...
        class="ai-check-fragment-popover flex gap-1 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 py-1 shadow"
      >
        ${this.state
          ? html`
              <button
